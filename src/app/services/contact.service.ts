...
export class ContactService {
  ...
  public loadContacts() {
    return from(storageService.query(ENTITY)).pipe(
      tap((contacts) => {
        ...
        console.log('service contactFilter:', contactFilter);
        contacts = this._filter(contacts, contactFilter.term, contactFilter.filterBy);
  ...
  private _sort(contacts: Contact[]): Contact[] {
    return contacts.sort((a, b) => {
      if (a.name && b.name && a.name.toLocaleLowerCase() < b.name.toLocaleLowerCase()) {
        return -1;
      }
      if (a.name && b.name && a.name.toLocaleLowerCase() > b.name.toLocaleLowerCase()) {
        return 1;
  ...
  private _filter(contacts: Contact[], term = '', filterBy = 'name') {
    term = term.toLowerCase();
    filterBy = filterBy.toLowerCase();
    return contacts.filter((contact) => {
      
        if (filterBy === 'name' && contact.name)
        return contact.name.toLowerCase().includes(term)
        else
        if (filterBy === 'email' && contact.email)
        return contact.email.toLowerCase().includes(term)
        else
        if (filterBy === 'phone' && contact.phone)
        return contact.phone.toLowerCase().includes(term)
